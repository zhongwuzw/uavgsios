...
- (void)viewDidLoad
{
    ...
    //加载背景图片
    UIImageView* yuzhou_bg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"yuzhou_2.jpg"]];
    ...
    self.windowRect = CGRectMake(0, 0, width, height);
    yuzhou_bg.contentMode = UIViewContentModeScaleToFill;
    [yuzhou_bg setFrame:self.windowRect];

    [self.view addSubview:yuzhou_bg];
    //计算小窗口位置
    POINT_1 = CGPointMake(2+18*(self.windowRect.size.width/2048)+self.windowRect.size.width/8, 2+73*(self.windowRect.size.height/4/210)+self.windowRect.size.height/8);
...
//            break;
    ...
    self.mapView = [[UIView alloc] initWithFrame:self.windowRect];
    UIImageView* yuzhou_bg2 = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"yuzhou_2.jpg"]];
    yuzhou_bg2.contentMode = UIViewContentModeScaleToFill;
    [yuzhou_bg2 setFrame:self.windowRect];
    [self.mapView addSubview:yuzhou_bg2];
    [self.mapView addSubview:baseViewC.view];
    [self.view addSubview:self.mapView];
    baseViewC.view.frame = self.windowRect;
...
//    [baseViewC setMaxLayoutObjects:1000];
    ...
    //加载主界面工具按钮
    self.CUS_BarController = [BarController new];
    ((CustomUIView *)(self.CUS_BarController.view)).passThroughViews = [NSArray arrayWithObject:self.view];
    ((CustomUIView *)(self.CUS_BarController.topBar)).passThroughViews = [NSArray arrayWithObject:self.view];
    [self.CUS_BarController.view setFrame:CGRectMake(0, 0, 1000, 600)];

//    [self.view addSubview:self.CUS_BarController.mapBar];
//    [self.CUS_BarController.mapBar setFrame:CGRectMake(500,200,60,500)];
//    
//    [self.view addSubview:self.CUS_BarController.warningBar];
//    [self.CUS_BarController.warningBar setCenter:CGPointMake(500, 500)];
    
    //加载主界面上方的工具条
    [self.view addSubview:self.CUS_BarController.topBar];
    float wid = CGRectGetWidth(self.windowRect);
    float hig = CGRectGetHeight(self.windowRect);
    NSLog(@"%fl",hig);
    NSLog(@"%fl",wid);
    [self.CUS_BarController.topBar setFrame:CGRectMake(0,
                                                       0,
                                                    self.windowRect.size.width,
                                                    self.windowRect.size.height/4
                                                       )];
    [self.CUS_BarController AutoSetViewPos:self.windowRect];
    ...
    self.liveCommandOperation = [ApplicationDelegate.liveCommandEngine startPreview:@"PV" symbol:@"02" onSucceeded:^(void){
        ...
        self.liveBroadView = [[LiveBroadView alloc] initWithFrame:CGRectMake(0,0, self.windowRect.size.width/4, self.windowRect.size.height/4)];
        [self.liveBroadView setVideoFillMode:AVLayerVideoGravityResizeAspectFill];
        self.liveBroadView.backgroundColor = [UIColor blackColor];
        [self.liveBroadView setCenter:POINT_1];
        self.liveBroadView.player = [AVPlayer playerWithURL:m];
        [self.liveBroadView.player addObserver:self forKeyPath:@"status" options:0 context:nil];
        [self.view addSubview:self.liveBroadView];
        self.liveBroadView.contentMode = UIViewContentModeScaleToFill;
    ...
    NSString *finalDirectory = [self.tmpDirectory stringByAppendingPathComponent:@"status.data"];
    
    [self.liveCommandOperation addDownloadStream:[NSOutputStream outputStreamToFileAtPath:finalDirectory append:NO]];
    
    self.liveDownloadTSOperation = [ApplicationDelegate.liveDownloadEngine fetchGPMediaJSON:^(NSMutableArray *GPMediaArray){
        self.GPMediaArray = [NSMutableArray arrayWithArray:GPMediaArray];
        NSLog(@"GPMediaArray is %@",GPMediaArray);
    }errorHandler:^(NSError *error){
        NSLog(@"commit error");
        DLog(@"%@\t%@\t%@\t%@",[error localizedDescription],[error localizedFailureReason],[error localizedRecoveryOptions],[error localizedRecoverySuggestion]);
    }];
    
    UIButton *toPhotoCollection = [UIButton buttonWithType:UIButtonTypeSystem];
    [toPhotoCollection setTitle:@"相册" forState:UIControlStateNormal];
    [toPhotoCollection addTarget:self action:@selector(handlePhotoCollection) forControlEvents:UIControlEventTouchUpInside];
    toPhotoCollection.frame = CGRectMake(10, 450, 30, 60);
    
    [self.view addSubview:toPhotoCollection];
    
    NSArray *tempArray = [[NSArray alloc] initWithObjects:@"test",@"haah",@"test",@"haah",@"test",@"haah",@"test",@"haah",@"test",@"haah",@"test",@"haah",@"test",@"haah",@"test",@"haah", nil];
    
    PlanScrollVIew *planScrollView = [[PlanScrollVIew alloc] initWithFrame:CGRectMake(100, 200, 800, 60) planArray:tempArray];
    
    [self.view addSubview:planScrollView];
    

    //测试网络
    local_net = [[UILabel alloc] initWithFrame:CGRectMake(400, 100, 200, 20)];
    www_net = [[UILabel alloc] initWithFrame:CGRectMake(400, 130, 200, 20)];
    [local_net setBackgroundColor:[UIColor redColor]];
    [local_net setTintColor:[UIColor whiteColor]];
    [local_net setText:@"本地网络待测试"];
    [www_net setBackgroundColor:[UIColor redColor]];
    [www_net setTintColor:[UIColor whiteColor]];
    [www_net setText:@"万维网待测试"];
    
    [self.view addSubview:local_net];
    [self.view addSubview:www_net];
    [self testNetConnet];
}

- (void)handlePhotoCollection{
    UICollectionViewFlowLayout *layOut = [[UICollectionViewFlowLayout alloc] init];
    layOut.itemSize = CGSizeMake(70, 70);
    layOut.scrollDirection = UICollectionViewScrollDirectionVertical;
    
    PhotoCollectionViewController *pcc = [[PhotoCollectionViewController alloc] initWithCollectionViewLayout:layOut];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    if (object == self.liveBroadView.player && [keyPath isEqualToString:@"status"]) {
        if (self.liveBroadView.player.status == AVPlayerStatusReadyToPlay) {
            [self.liveBroadView.player play];
            
        }
        else if (self.liveBroadView.player.status == AVPlayerStatusFailed)
        {
            NSLog(@"AVPlayer play failed");
        }
        else if (self.liveBroadView.player.status == AVPlayerStatusUnknown)
        {
            NSLog(@"Don't konw player's status");
        }
    }
}
...
+ (TestViewController *)shared{
    static TestViewController* sharedTestViewController = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken,^{
        sharedTestViewController = [[TestViewController alloc] initWithMapType:MaplyGlobe];
    
    });
    return sharedTestViewController;
}
- (void)swithMode{
    ...
    if (self.mode==0) {
        [UIView animateWithDuration:0.5 animations:^{
            //地图放大
            CGAffineTransform tran = self.mapView.transform;
            tran = CGAffineTransformScale(tran, 4, 4);
            self.mapView.transform = tran;
            [self.mapView setCenter:CGPointMake(self.windowRect.size.width/2, self.windowRect.size.height/2)];
            //视频缩小
            CGAffineTransform tran_video = self.liveBroadView.transform;
            tran_video = CGAffineTransformScale(tran_video, 0.25, 0.25);
            self.liveBroadView.transform = tran_video;
            //[self.liveBroadView setCenter:CGPointMake(self.windowRect.size.width/8+5, self.windowRect.size.height - self.windowRect.size.height/8 - 8)];
            [self.liveBroadView setCenter:POINT_1];
            //切换显示层次
            [self.view bringSubviewToFront:self.liveBroadView];
            [self.view bringSubviewToFront:self.CUS_BarController.topBar];
        ...
        [UIView animateWithDuration:0.5 animations:^{
            //地图缩小
            CGAffineTransform tran = self.mapView.transform;
            tran = CGAffineTransformScale(tran, 0.25, 0.25);
            self.mapView.transform = tran;
            //[self.mapView setCenter:CGPointMake(self.windowRect.size.width/8+5, self.windowRect.size.height - self.windowRect.size.height/8-5)];
            [self.mapView setCenter:POINT_1];
            //视频放大
            CGAffineTransform tran_video = self.liveBroadView.transform;
            tran_video = CGAffineTransformScale(tran_video, 4, 4);
            self.liveBroadView.transform = tran_video;
            [self.liveBroadView setCenter:CGPointMake(self.windowRect.size.width/2, self.windowRect.size.height/2)];
            //切换显示层次
    ...
    if (www_net&&local_net) {
        [self.view bringSubviewToFront:www_net];
        [self.view bringSubviewToFront:local_net];
    }
}
-(void)testNetConnet{
    MKNetworkEngine *engin = [MKNetworkEngine new];
    MKNetworkOperation *www_op = [[MKNetworkOperation alloc] initWithURLString:@"http://www.baidu.com" params:nil httpMethod:@"GET"];
    [www_op onCompletion:^(MKNetworkOperation *completedOperation) {
        [www_net setText:@"万维网已通"];
    } onError:^(NSError *error) {
        [www_net setText:@"万维网不通"];
    }];
    
    //测试外网地址
    
    //测试本地地址
    MKNetworkOperation *local_op = [[MKNetworkOperation alloc] initWithURLString:@"http://10.5.5.160:8090/weixinAC?echostr='abc'" params:nil httpMethod:@"GET"];
    [local_op onCompletion:^(MKNetworkOperation *completedOperation) {
        [local_net setText:@"本地已通"];
    } onError:^(NSError *error) {
        [local_net setText:@"本地未通"];
    }];
    [engin enqueueOperation:www_op];
    [engin enqueueOperation:local_op];
}
@end
